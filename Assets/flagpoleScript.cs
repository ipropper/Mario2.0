...
public class flagpoleScript : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D other){
		...
		if(other.tag == "Player"){
			...
			if      (poleHeight >= 154)GuiValues.numLives += 1;
			else if (poleHeight >= 128)GuiValues.points += 5000;
			else if (poleHeight >= 82) GuiValues.points += 2000;
			else if (poleHeight >= 58) GuiValues.points += 800;
			else if (poleHeight >= 18) GuiValues.points += 400;
			else 					   GuiValues.points += 200;
